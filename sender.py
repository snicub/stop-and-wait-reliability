...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while last_acked < final_ack - 1:
        ack_window = 0
        ack_size = 0
        #want to wait of rinocming data
        ready = select.select([cs], [], [], RTO)
        if ready[0]:  
            #data is vailabel to be read
            ack_data, _ = cs.recvfrom(4096)
            ack_msg = Msg.deserialize(ack_data)
            print(f"[S] Received ACK: {str(ack_msg)}")


            if ack_msg.ack > last_acked:
            #need ot update the ack
                ...
                win_right_edge = min(win_left_edge + win_size, final_ack)
               #nowto transimti entire window 
                transmit_entire_window_from(last_acked)
        else:
            #we have a timoutee
            print(f"[S] Timeout occurred, retransmitting from sequence {win_left_edge}")
            transmit_one()

    print("[S] All packets successfully transmitted and acknowledged.")
